...
sys.path.insert(0, "../")
import scgpt as scg
from scgpt.model.model_default import TransformerModel, AdversarialDiscriminator # you can add it to __init__.py to organize better
...
from scgpt.utils import set_seed, category_str2int, eval_scib_metrics,add_file_handler
import random
...
for r in range(_RUN_COUNT):
    ...
    save_dir = Path(f"./save_scgcn/scgpt_{dataset_name}_run_{str(r)}/")
    ...
    def prepare_data(sort_seq_batch=False) -> Tuple[Dict[str, torch.Tensor]]:
        ...
        print("**********",train_data_pt["values"].size())
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save(train_data_pt, save_dir / "train_loader.pth")
        torch.save(valid_data_pt, save_dir / "valid_loader.pth")   
    ...
    def test(model: nn.Module, adata: DataLoader) -> float:
        ...
        torch.save(test_data_pt, save_dir / "test_loader.pth") # save test data( the data enters the dataloader)
    ...
    ## If we have the id2type, predictions and labels ( that we stored ), we can easily plot the 
    from sklearn.metrics import confusion_matrix
    ...
    ##############################################################################################
    # save the model into the save_dir
    torch.save(best_model.state_dict(), save_dir / "model_ckpt.pt")
