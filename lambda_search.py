...
if __name__ == "__main__":
    ...
    cur_dir = os.getcwd()
    save_dir = cur_dir + "/scarce_lambda"
    if not os.path.exists(path=save_dir):
        save_dir = os.path.join(cur_dir, 'scarce_lambda')
    ...
    lambda_list = [0.9, 0.8, 0.7, 0.6, 0.5]
    ...
    #### Take results from the saved transformer model. Only used to look up the seed list.
    ...
    for l in lambda_list:
        ...
        params = SearchParams(
            ...
            wd=0.00005,
            patience=10,
            max_epochs=50,  # if batch_size is 32, then use max_epochs as 50
            encoder_lr=0.00001, train_fraction=0.5,
            encoder_p=0.2, batch_size=8
        )
        ...
        for pt in path_list:
            ...
            load_dir = save_dir + "/" + f"{dataset_name}/{type_name}/portion_{int(params.train_fraction*100)}/lambda_{str(l)}/{pt}/"
            ...
            for i, seed in enumerate(seed_list[2:5]):
                ...
                d["test_preds"].append(trainer.y_test_preds)
                d["test_true"].append(trainer.y_test_true)
                ...
                result_dir = os.path.join(
                    load_dir,
                    f"dname_{dataset_name}_path_[{pt}]_type_{type_name}_seedid_{str(i)}_seed_{seed}_lambda_{l}_portion_{params.train_fraction}"
                )
            ...
            print(f"[lambda={l}] Accuracy", sum(d["test_acc"])/len(d["test_acc"]))
            print(f"[lambda={l}] F1", sum(d["test_f1"])/len(d["test_f1"]))
            print(f"[lambda={l}] Accuracy STD", np.std(np.array(d["test_acc"])))
            print(f"[lambda={l}] F1 STD", np.std(np.array(d["test_f1"])))

            if len(d["best_test_acc"]) > 0:
                print(f"[lambda={l}] Best Accuracy", sum(d["best_test_acc"])/len(d["best_test_acc"]))
                print(f"[lambda={l}] Best F1", sum(d["best_test_f1"])/len(d["best_test_f1"]))
                print(f"[lambda={l}] Best Accuracy STD", np.std(np.array(d["best_test_acc"])))
                print(f"[lambda={l}] Best F1 STD", np.std(np.array(d["best_test_f1"])))
                print(f"[lambda={l}] Best Epochs", d["best_test_epoch"])
